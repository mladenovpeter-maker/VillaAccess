...
**Aspect-rejection is a SYMPTOM of bad angle, NOT a config-fixable cause.** `ANPR_MIN_BOX_ASPECT` (env, default 1.8; one deployment set 1.50) rejects boxes whose w/h is too square — an oblique/fisheye plate yields a near-square box (~1.34). Tested lowering it to 1.2 on the real Портал Юг frame: box then PASSED to OCR but Tesseract read a single junk char ('A') from the small (~165×97px after shrink) tilted crop. **Why:** the pipeline has no deskew/perspective-correct step, so a foreshortened/warped plate is unreadable regardless of threshold. **How to apply:** don't chase the aspect threshold to "fix" a camera that sees plates at an angle — the only fix is physical head-on re-aim so the box is naturally a wide rectangle. Quick zero-risk test before/after re-aim: `docker compose exec -T -e ANPR_MIN_BOX_ASPECT=1.2 ai-worker python -c "import worker; print(worker.ocr_plate(open('/tmp/<frame>.jpg','rb').read(), camera_id='TEST'))"`.
