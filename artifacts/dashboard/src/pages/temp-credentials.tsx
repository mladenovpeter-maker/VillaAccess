...
// datetime-local inputs are naive *local* wall-clock; format from local
// components (not toISOString, which is UTC) so a server instant round-trips
// without an offset shift. Submit paths convert back via new Date(...).toISOString().
...
function EditCredDialog({ cred, onClose }: { cred: TempCred | null; onClose: () => void }) {
  const qc = useQueryClient();
  const { toast } = useToast();
  const { t } = useTranslation();

  const isStandalone = !!cred && !cred.reservation_id;
  const isPermanent = isStandalone && cred?.access_type === "permanent";
  const showDates = !isPermanent;
  ...
  useEffect(() => {
    if (!cred) return;
    setOwner(cred.owner_name ?? "");
    setLabel(cred.label ?? "");
    setNotes(cred.notes ?? "");
    setValidFrom(toInputDateTime(new Date(cred.valid_from)));
    setValidUntil(toInputDateTime(new Date(cred.valid_until)));
  }, [cred]);
  ...
  const mut = useMutation({
    mutationFn: () => {
      ...
      if (isStandalone) payload.owner_name = owner || undefined;
      if (showDates && cred) {
        // Only send a window field when it actually changed, so editing
        // label/notes never rewrites the validity window or triggers an
        // unnecessary device re-sync.
        const fromISO = new Date(validFrom).toISOString();
        const untilISO = new Date(validUntil).toISOString();
        if (fromISO !== new Date(cred.valid_from).toISOString()) payload.valid_from = fromISO;
        if (untilISO !== new Date(cred.valid_until).toISOString()) payload.valid_until = untilISO;
      }
      return api.patch(`/temp-credentials/${cred!.id}`, payload);
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["temp-credentials"] });
      toast({ title: t("tempCreds.updated") });
      onClose();
    },
    onError: (e: Error) => toast({ title: t("common.error"), description: e.message, variant: "destructive" }),
  });
  ...
  const canSubmit = isStandalone
    ? !!owner && (isPermanent || (!!validFrom && !!validUntil))
  ...
  return (
    <Dialog open={!!cred} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-md w-[95vw]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Pencil className="w-4 h-4" />{t("tempCreds.editAccess")}
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-4 py-2">
          {isStandalone && (
          ...
          {/* PIN is immutable — shown read-only for reference */}
          <div className="space-y-1.5">
            <Label>{t("tempCreds.pinCode")}</Label>
            <Input value={cred?.pin_code ?? ""} disabled className="font-mono tracking-widest" />
          </div>
...
export default function TempCredentialsPage() {
  ...
  const [editTarget, setEditTarget] = useState<TempCred | null>(null);
  ...
  return (
    ...
    >
      ...
      <EditCredDialog cred={editTarget} onClose={() => setEditTarget(null)} />
